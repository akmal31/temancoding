...
function TopUpContent() {
  ...
  const handleBuy = (credits: number) => {
    // Mocking payment success instantly
    addCredits(credits);
    if (redirect) {
    ...
    <main className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center py-20 px-6">
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-5xl font-bold font-space mb-4">Isi Credit</h1>
          <p className="text-zinc-400 max-w-lg mx-auto">
            1 Credit = 1x generate Arsitektur dan PRD secara lengkap. Beli paketan biar lebih hemat.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-5xl">
          {packages.map((pkg, i) => (
            ...
            >
              ...
              <div className="mb-8">
                <h3 className="text-2xl font-space font-bold mb-2">{pkg.name}</h3>
                <div className="flex items-center gap-2 mb-1">
                  <Coins className="w-5 h-5 text-amber-400" />
                  <span className="text-xl font-medium">{pkg.credits} Credit</span>
                </div>
              </div>
              
              <div className="mb-8 flex-1">
                <div className="flex flex-col">
                  <span className="text-zinc-500 line-through text-sm">{pkg.originalPrice}</span>
                  <span className="text-4xl font-bold font-space">{pkg.price}</span>
                </div>
              </div>
