    ...
    async signIn({ user, account, profile }) {
      if (account?.provider === "google") {
        try {
          ...
          return true; // Still allow sign in to proceed even if db recording fails? Better to fail.
          // Wait, if db fails, the preview environment might not have DB setup yet. We should let them sign in so UI doesn't completely break without DB
        }
      }
      return true;
    },
